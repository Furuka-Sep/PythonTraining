...
def btn1():
    ...
    if count == 9:
        if ans == EN[count]:
            score += 1
            judge['text']="正解!\nあなたは10門中{}問正解しました!".format(score)
        else:
            judge['text']="不正解!\n答えは{}\nあなたは10門中{}問正解しました!".format(EN[count],score)
    elif ans == EN[count]:
        score += 1
        judge['text']="正解!"
    else:
    ...
    imgl = tk.PhotoImage("lion.png")
    imgl = img.subsample(2)
    canvas.create_image(300,100,image=imgl,tag="animal")
    ja["text"]=JA[count]
root = tk.Tk()
